...
> 2026-09-15 后续变更：用户要求 `study-log` 全部导出位于当前仓库内，并按结构化记录与同名原文一一配对；现役规则见 [study-log](../skills/study-log/SKILL.md)。下文关于仓库外私有根和外置临时提取的决策保留为当时事实，不再约束新导出。隐私确认、来源完整性、覆盖防护和独立 Git 授权继续有效；本次只迁移用户指定的 PlanA 归档，不自动改变其他消费仓库。
...
## D41：`guide-learning` 按需学习文章产物与历史产物适配边界（2026-08-12）

用户确认学习文章是教学完成后的可选综合产物，归 `guide-learning` 所有，不等待也不并入延期中的
`creator-workflow`。采用“中央通用骨架＋受限消费配置”的结构：

- 中央文章骨架只定义从来源与范围、问题或目标、主题理解到回顾／证据／应用／开放问题的通用组织方式；
  消费仓库的可选 `article_profile` 只可选择语言、语气、必选／可选章节、领域视角和候选目标，不得注入
  新流程、模板正文、状态值或授权文本。没有该配置时，既有教学流程保持兼容，不自动生成文章。
- 文章采用双授权：用户先请求或确认本次起草范围，Agent 才能形成草稿；保存时还必须再次确认精确目标
  与覆盖边界。静态 profile、目标目录和 write allowlist 都不构成写入、覆盖、发布、提交或推送授权。
- 文章只综合已经确认的学习内容、练习证据和来源，不回写或拥有 Program、Lesson、Session event、
  Checkpoint、mastery 等学习状态；也不复制 `study-log` 的过程记录、对话提取或 raw archive。信息不足
  时保留边界和待确认项，不把推断冒充学习事实。

PlanA 的首轮历史产物适配边界同时确认：

- `计划/记忆` 中重复的 AI 陪学交互规范、独立陪学流程和仓库内文章模板，在中央行为与 PlanA 配置层
  完成承接后退役；真正属于 PlanA 的路径、状态所有者、时间归属、领域视角和文章目标只保留在配置层。
- 当前活动状态只建立稀疏且可验证的 Program → Lesson → Checkpoint 链。长期主计划与临时冲刺计划的
  关系、当前 Lesson、session event 索引和唯一下一动作分别由明确所有者承担；进度总表只是派生视图。
  迁移不得追溯猜测缺失的授权、实际用时、正式练习接受、六维 mastery 或结课结论。
- programming-lab 已完成的 Lesson 01／02、10 份历史对话、实验及相关 legacy 产物继续冻结，不做
