...
3. 学前预测与当前理解诊断（D11 已确定必要前置核对与条件性预测）；
4. 讲解、提问和纠错顺序（D4 已确定默认微循环，D12 已确定检查题与复查规则）；
5. 微动作、正式练习与提示梯度（D5 已确定条件分流和求助控制，D12 已确定自然语言提示梯度；
   最小练习契约的最终字段与场景模板仍待设计）；
6. 编码练习中的测试与作品所有权（D7 已确定契约范围授权和核心工件保护）；
7. 作品审查、修订和问题关闭（D7 已确定精简 finding 与停止规则）；
8. 理解检查、迁移检查和完成门槛（D5 已确定三维 mastery，D12–D13 已确定检查形式、适用维度和
   最低充分证据）；
