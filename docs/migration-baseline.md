...
- 2026-08-11 启动 M5：完成门槛复核后，中央源码树删除两个 rollback-only 旧目录，把 `learn-by-practice` 与 `study-companion` 移入 `retired_names` 并继续指向 `guide-learning`；消费者指针升级与最终提交号将在本阶段后续记录。
