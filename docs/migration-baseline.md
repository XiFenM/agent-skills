...
> 2026-09-15 的 `study-log` 仓库内同名配对规则已替代外置新日志的旧约定，见[当前 Skill](../skills/study-log/SKILL.md)。下方旧导出器与外置归档描述仍是历史事实，不据此迁移用户未授权的旧记录。
...
3. `creator-workflow` 的中央通用化与 `daily-work` 的 version 2 消费适配、旧行为副本清理、历史保护和
   全新检出验证均已完成候选；当前只剩按依赖顺序提交并发布中央仓库与 `daily-work`。
